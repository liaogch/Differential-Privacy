...
%epsilon_max = 0.0183;
prospect_val = zeros(1,length(epsilon_max));
...
for m = 1:length(epsilon_max)
    ...
    for i = 1:n
        ...
        if epsilon > epsilon_ref
            sum(m) = sum(m) + Weighting_Fun( p, u )* Valuation_Fun( epsilon-epsilon_ref,beta,lambda);
        else
            sum(m) = sum(m) + 0;
        end
    end

    prospect_val(m) = sum(m) / norm(m);

    
    G(m) = c * prospect_val(m);
